...
# NOTE: match the Roboflow notebook / known-good model version.
...
class NumberRecognizer:
    ...
    Uses Roboflow hosted API for OCR on cropped jersey regions.
    """
    ...
    def recognize_crops(self, crops: List[np.ndarray]) -> List[str]:
        ...
        for crop in crops:
            try:
                ...
                # Notebook uses: MODEL.predict(crop, prompt)[0]
                result = self.model.predict(resized, NUMBER_OCR_PROMPT)

                # `predict` usually returns a list[str]; be defensive.
                if isinstance(result, list) and result:
                    text = str(result[0])
                else:
                    text = str(result)

                numbers.append(_normalize_number_text(text))
...
def _normalize_number_text(text: str) -> str:
    """
    Normalize model output to a jersey-number-like digit string.
    - Strips whitespace
    - Extracts digits (e.g. 'Number: 23' -> '23')
    """
    t = (text or "").strip()
    digits = re.findall(r"\d+", t)
    if not digits:
        return ""
    # Join groups in case output has separators, e.g. "2 3"
    return "".join(digits)
    ...
    for player_idx, number_idx in pairs:
        ...
        if number_str:  # Only record non-empty reads
            ...
            readings.append(NumberReading(
                ...
                confidence=float(
                    number_dets.confidence[number_idx]) if number_dets.confidence is not None else 1.0
            ))
