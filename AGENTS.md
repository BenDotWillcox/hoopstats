...
uv run hoopstats map-court-video --video clip.mp4 --out data/outputs --tracker sam2  # GPU SAM2 court map (short clips; see notebooks/hoopstats_colab.ipynb)
...
uv run hoopstats process-game --video game.mp4 --out data/outputs --segments segs.csv  # uncut broadcast: in-scope ranges only
uv run hoopstats demo --video data/raw/clip.mp4 --out data/demo  # reproducible demo + report
uv run hoopstats annotate --video path/to/video.mp4 --out data/labels/gt.csv  # label ground truth (mark FTs with zone 'ft')
uv run hoopstats mark-segments --video path/to/video.mp4 --out data/labels/segs.csv  # mark live-play ranges (i=in, o=out)
uv run hoopstats evaluate --shots shots.csv --labels gt.csv --segments segs.csv  # score predictions vs labels
uv run hoopstats report --frames-dir data/outputs --stats-dir data/outputs/game --labels gt.csv --segments segs.csv  # self-contained portfolio HTML walkthrough

# Run unit tests (event detector + CSV export; no models/API needed)
uv run pytest
```
